class ShapeMaker {
  ...
  private PVector[][] calcVerticesCylinder(int baseSegments) {
    ...
    for (int i = 0; i < decks; i++) {
      verticesArray[i][0]=new PVector(0, 0, shapeHeight/sideSegments*i-(shapeHeight/2));
    }
    for (int i = 1; i <= baseSegments; i++) {
      ...
      for (int j = 0; j < decks; j++) {
        verticesArray[j][i]=new PVector(x, y, shapeHeight/sideSegments*j-(shapeHeight/2));
      }
    }
  ...
  private PVector[][] calcVerticesCone(int baseSegments) {
    ...
    //Nullpunkt
    verticesArray[0][0]=new PVector(0, 0, 0);
    //Spitze der Cone
    verticesArray[1][0]=new PVector(0, 0, shapeHeight);

    for (int i = 1; i <= baseSegments; i++) {
      float a = radians(360/baseSegments*i); 
      float x = 0 + shapeRadius * cos(a); 
      float y = 0 + shapeRadius * sin(a);
      verticesArray[0][i]=new PVector(x, y, 0);
    }
    return verticesArray;
  }
